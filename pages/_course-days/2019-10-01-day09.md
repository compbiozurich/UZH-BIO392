...
Please note exercises 33 depict an integrative analysis of human genetic variation (mobile element insertions) and might require a longer time (e.g. tomorrow session). Even though they can be solved in relatively few lines of standard UNIX tools (`curl`, `head`, `wc`, `awk`) and bioinformatics software (`vcftools`), they are complex, as they process relatively big amounts of data.
