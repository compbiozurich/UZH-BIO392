...
Pooled_byTP53 <- survfit(Surv(info.followupMonths, info.death) ~ tp53_status, data = masterlist)
ggsurvplot(
  Pooled_byTP53, conf.int="TRUE", xlab = "Follow-up in months", ylab="Survival probability", 
  legend.title = "TP53 status", legend.labs = c("Deletion", "Wildtype"), title = "Survival probability by mutation status of TP53", 
  ggtheme = theme_bw())

Pooled_byMYC <- survfit(Surv(info.followupMonths, info.death) ~ myc_status, data = masterlist)
ggsurvplot(
  Pooled_byMYC, conf.int="TRUE", xlab = "Follow-up in months", ylab="Survival probability", 
  legend.title = "MYC status", legend.labs = c("Duplication", "Wildtype"), title = "Survival probability by mutation status of MYC", 
  ggtheme = theme_bw())

Pooled_byERBB2 <- survfit(Surv(info.followupMonths, info.death) ~ erbb2_status, data = masterlist)
ggsurvplot(
  Pooled_byERBB2, conf.int="TRUE", xlab = "Follow-up in months", ylab="Survival probability", 
  legend.title = "ERBB2 status", legend.labs = c("Duplication", "Wildtype"), title = "Survival probability by mutation status of ERBB2", 
  ggtheme = theme_bw())

Pooled_byCDKN2A <- survfit(Surv(info.followupMonths, info.death) ~ cdkn2a_status, data = masterlist)
ggsurvplot(
  Pooled_byCDKN2A, conf.int="TRUE", xlab = "Follow-up in months", ylab="Survival probability", 
  legend.title = "CDKN2A status", legend.labs = c("Deletion", "Wildtype"), title = "Survival probability by mutation status of CDKN2A", 
  ggtheme = theme_bw())
