...
#' @param ld A data.frame with minimum CHROM_A, BP_A, CHROM_B, BP_B and R2.
...
## LD decay plot
ld <- read_table2("ALL.maf.decay.ld") 
pdf('out/ALL.maf.decay.pdf', width = 6, height = 5)
plotDecayLD(ld,xlim = c(0,200), ylim = c(0,1), avgwin = 200, title = 'LD decay All')
dev.off()
...
ld <- read_table2("AFR.maf.Link.ld") 
ld <- ld %>% filter(BP_A > start  & BP_A < end  & ld$BP_B > start  & ld$BP_B < end )
pdf('out/AFR.maf.ld.pdf', width = 18, height = 5)
plotPairwiseLD(ld ,ylim=c(0,300), xbreaks = 10, title = 'Linkage plot Afr')
dev.off()

ld <- read_table2("HAN.maf.Link.ld") 
ld <- ld %>% filter(BP_A > start  & BP_A < end  & ld$BP_B > start  & ld$BP_B < end )
pdf('out/HAN.maf.ld.pdf', width = 18, height = 5)
plotPairwiseLD(ld ,ylim=c(0,300), xbreaks = 10, title = 'Linkage plot Han')
dev.off()
